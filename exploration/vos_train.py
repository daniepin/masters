...
def train(
    ...
    if state["vos_enable"]:
        data_dict = torch.zeros(state["num_classes"], state["sample_number"], state["output_channels"]).to(device)
        number_dict = {}
        ...
        for data, target in train_loader:
            ...
            if sum_temp == state["num_classes"] * state["sample_number"] and epoch < start_epoch:
            ...
            elif sum_temp == state["num_classes"] * state["sample_number"] and epoch >= start_epoch:
                # Convert the target tensor to a NumPy array on the CPU
                target_numpy = target.cpu().data.numpy()
