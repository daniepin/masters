...
from transforms import ukb_transforms
...
def main():
    ...
    val_loader = monai.data.DataLoader(
        val_dataset, batch_size=state["batch_size"], num_workers=4, pin_memory=torch.cuda.is_available()
    )
    ...
    if state["vos_enable"]:
        vos_params = {
            ...
            "log_reg": torch.nn.Sequential(torch.nn.Linear(1, 12), torch.nn.ReLU(), torch.nn.Linear(12, 2))
        }
        
        torch.nn.init.uniform_(vos_params["weight_energy"].weight)
        vos_params["log_reg"].to(device)
        
        optim_params = list(vos_params["weight_energy"].parameters()) + list(vos_params["log_reg"].parameters())
